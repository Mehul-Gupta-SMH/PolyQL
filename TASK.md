...
|------|--------|------|--------|
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `benchmark/ingest_bird.py` | Fix: column description field priority changed to `column_description → value_description → description`; BIRD CSVs use `column_description` as main field — `value_description` is sparse/empty for most columns |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `benchmark/ingest_bird.py` | Fix: Windows-1252 encoding fallback — 3 BIRD CSVs (`qualifying.csv`, `Player_Attributes.csv`, `Budget.csv`) contain 0x95/0x96 bytes invalid in UTF-8; added `latin-1` retry on `UnicodeDecodeError` |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `benchmark/run_inference.py` | Fix: resume logic now only skips `result_type=sql` rows; error rows are retried on resume; rewrites JSONL file stripping errors before starting (import: added `write_jsonl` to imports from `_bench_utils`) |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | Kuzu instances (11 × `bird_*`) | Fix: all 11 `bird_*` Kuzu graph files deleted and force-re-ingested — each contained ~12 extra Northwind tables (categories, orders, employees, etc.) that leaked from `default` instance during prior development; contamination caused incorrect JOIN paths for BIRD queries |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | BIRD benchmark run | First 500q run hit Claude Code CLI daily quota at 374/500 questions (error: "You've hit your limit · resets 9:30pm IST"); 126/500 actually ran — preliminary EX: debit_card_specializing 23%, student_club 60%, thrombosis_prediction 36% |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | Cron job (ID: b570b29d) | Created recurring cron job firing at 9:35pm IST daily — runs `python -m benchmark.run_inference --benchmark bird --data_dir ... --provider claude_code --output_file results/bird_mini.jsonl --instance_prefix bird --sleep 1`; resumes from last successful question automatically |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `benchmark/ingest_agent.py` | BM4: New LLM-powered universal benchmark ingestion agent — uses Anthropic SDK manual tool-use loop with 4 tools (`list_directory`, `read_file`, `store_table`, `clear_instance`); auto-detects BIRD/Spider/SQL/CSV/JSON schema formats; CLI: `python -m benchmark.ingest_agent --data_dir ... --instance_prefix ...` |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `pyproject.toml` | Added `benchmark` optional extra (lists `anthropic>=0.21` for ingest_agent) |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `CHANGELOG.md` | Added BM4 entry for universal ingestion agent |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `README.md` | Full update: added Business Semantic Layer section (glossary API, term injection), Benchmark Evaluation section (BIRD 3-step workflow + universal agent CLI), updated Features table (SL + benchmark rows), updated test count to 247, updated Project Layout (benchmark/, validation/, GlossaryStore.py, backend/glossary.py) |
| 2026-03-31 | `Claude/feature/validation-outcome-layer` | `DEVELOPER.md` | Major rewrite: added Kuzu Graph DB section (schema, operations, migration, `_merge_node` importance), Business Semantic Layer (architecture flow + REST API table), Benchmark Harness (3-step workflow, EX/VES metrics, evidence injection, universal agent, known issues), updated Storage backends table, API endpoints table, test count to 247 with per-area breakdown, Observability section |
| 2026-03-22 | `Claude/feature/validation-outcome-layer` | `MetadataManager/GlossaryStore.py` | SL0+SL1: `business_terms` SQLite table + full CRUD (`add_term`, `get_term`, `get_term_by_name`, `update_term`, `delete_term`, `list_terms`, `search_by_name`) + `index_term()` ChromaDB embed in `business_glossary` collection + `get_business_context()` cosine-similarity semantic search with distance-threshold gating |
| 2026-03-22 | `Claude/feature/validation-outcome-layer` | `main.py` | SL2: `_get_business_context()` wrapper; called in `generateQuery`, `generateQueryStream`, `gatherRequirements`; attaches `glossary_hits` to context dict before `format_schema` |
| 2026-03-22 | `Claude/feature/validation-outcome-layer` | `APIManager/PromptBuilder.py` | SL2: `format_schema()` renders `## Business Definitions` block from `context['glossary_hits']` — term name, formula, table deps, example value — prepended before schema section |
| 2026-03-22 | `Claude/feature/validation-outcome-layer` | `backend/glossary.py`, `backend/app.py` | SL3: Glossary REST API (`APIRouter`): `POST /api/glossary/terms/single`, bulk, `GET`, `PUT`, `DELETE`, `GET /api/glossary/search?q=`; CORS updated to allow `PUT` |
| 2026-03-22 | `Claude/feature/validation-outcome-layer` | `tests/test_glossary_store.py`, `tests/test_glossary_retrieval.py` | SL6: 35 CRUD unit tests + 19 semantic/injection tests; all 247 suite tests pass |
| 2026-03-31 | `Claude/feature/benchmark-harness` | `backend/ingestion.py` | Fix: store_table now calls _merge_node so tables with no FK are added as Kuzu nodes (was causing NodeNotFound on JOIN path lookup) |
| 2026-03-31 | `Claude/feature/benchmark-harness` | `benchmark/run_inference.py` | Fix: mini_dev_sqlite.json added to question file search list; composite PK handling in ingest_bird.py/ingest_spider.py |
